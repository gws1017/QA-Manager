...
export async function GET() {
  ...
  const profile = getRegistry()
    .prepare('SELECT email, email_verified, display_name, notify_assigned, notify_status_change, notify_issue_changed FROM user_profiles WHERE user_id=?')
    .get(r.userId) as { email: string; email_verified: number; display_name: string | null; notify_assigned: number; notify_status_change: number; notify_issue_changed: number } | undefined;
