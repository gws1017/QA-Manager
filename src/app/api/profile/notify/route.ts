...
export async function PATCH(req: Request) {
  ...
  const { notify_assigned, notify_status_change, display_name } = await req.json();
  const reg = getRegistry();
  if (display_name !== undefined) {
    reg.prepare(`UPDATE user_profiles SET display_name=? WHERE user_id=?`).run(display_name || null, r.userId);
  }
  if (notify_assigned !== undefined || notify_status_change !== undefined) {
    reg.prepare(`UPDATE user_profiles SET notify_assigned=?, notify_status_change=? WHERE user_id=?`)
