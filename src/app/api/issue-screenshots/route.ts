...
export async function GET(req: Request) {
  ...
  const rows = getDb().prepare('SELECT * FROM issue_screenshots WHERE issue_id=? ORDER BY id').all(issueId);
...
export async function POST(req: Request) {
  ...
  const result = getDb().prepare('INSERT INTO issue_screenshots (issue_id, filename) VALUES (?,?)').run(issueId, filename);
